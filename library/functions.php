...
function buildProductsDisplay($products){
   ...
   function buildDetailDisplay($productDetails){
    ...
    $pdetails .= "</ul>";      
    $pdetails .= "<p class ='price'><strong>$$productDetails[invPrice]</strong></p>";
    $pdetails .= "</div>";
    return $pdetails;
    
}

function buildProductImageDisplay($productImages) {
    // print_r ($productImages);
    $id = '<ul class="imageBox">';
    foreach ($productImages as $image) {
        $id .= '<li>';
        $id .= "<img class='productimages' src='$image[imgPath]' title='$image[imgName] image on Acme.com' alt='$image[imgName] image on Acme.com'>";
        $id .= '</li>';
    }
    $id .= '</ul>';
    return $id;
...
function resizeImage($old_image_path, $new_image_path, $max_width, $max_height) {
   ...
   function buildProductsReviews($reviews){
    ...
    //   $pd .= "<h3>Posted by $review[clientFirstname]</h3>";
      $pd .= "<h2>Posted by $review[clientFirstname] on ".$d."</h2>";
